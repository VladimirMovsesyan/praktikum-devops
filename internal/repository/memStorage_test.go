...
func TestMemStorage_Delete(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				newMetric: metrics.NewMetricCounter("first", 1),
			},
			want: fields{
				mtrcs: []metrics.Metric{
					metrics.NewMetricCounter("second", 2),
		...
		},
		{
			...
			args: args{
				newMetric: metrics.NewMetricGauge("first", 1),
			},
			want: fields{
				mtrcs: []metrics.Metric{
					metrics.NewMetricCounter("first", 1),
